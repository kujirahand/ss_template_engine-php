...
class ss_template {
  ...
  public function show($name, $params = null) {
    ...
    $f_cache = $this->dir_cache.'/'.$name.'.ss.cache.php';
    // check cache
    ...
    $tpl = preg_replace_callback(
      ...
      function ($m) use (&$params, $plugins, $page) {
        ...
        foreach ($cmd_list as $cmd) {
          ...
          // string
          $ch1 = substr($cmd, 0, 1);
          $ch2 = substr($cmd, strlen($cmd) -1, 1);
          if ($ch1 == $ch2 && ($ch1 == '"' || $ch1 == "'")) {
            $res = ($res == "") ? $cmd : "{$res}\.{$cmd}";
            continue;
          }
          // other
          // throw new TemplateException("unknown command `$cmd`", $page);
          $res .= "";
        }
        // add php tag
        if ($res == "") return "";
        if ($raw_flag == false) $res = "ss_template_plugin_escape($res)";
        if ($echo_flag) $res = "echo $res;";
        return "<?php $res ?>";
      }, $tpl);
    $gen = "<?php /* `$page` generated by $page.ss.html with ss_template.php */ ?>\n";
    file_put_contents($f_cache, $gen.$tpl);
  }
}
...
function ss_template__replace_var_name($ar) {
  $ar = preg_replace_callback('#([a-zA-Z_][a-zA-Z0-9_\.]*)#',
    function($m){
      $args = explode('.', $m[1]);
...
function ss_template_plugin_eval($v) {
  $src = "return ($v);";
  return eval($src);
}
